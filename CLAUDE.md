...
|---------|------|---------|----------------|
| 0.1.20 | 2026-04-24 | **New batch-inference Colab notebook.** After the four-cell ablation table closed on 2026-04-23 (see `RESULTS.md`, `PAPER.md`), we still only had per-model numbers on the 25-class mapped PlantDoc subset (n=153). Added `notebooks/PhasePhyto_Batch_Inference.ipynb` to run any combination of trained checkpoints over **every** target image (PlantDoc or Cassava), not just the mapped subset. 28 cells (16 code, 12 markdown): (1) embedded how-to-run guide as cell 0, (2) dependency install + Drive mount + repo clone (cells 2-4), (3) single editable CONFIG cell (cell 6) with `MODEL_CHECKPOINTS` list of `{name, ablation, path}` dicts (placeholders left commented out), `DATASET_KIND ∈ {plantdoc, cassava, custom}`, `CLASS_TO_IDX_SOURCE` (json / PV root / .pt checkpoint), output dir, inference knobs. (4) Inlined ablation-aware `PhasePhyto` class (cell 9) that imports `LogGaborFilterBank`, `PhaseCongruencyExtractor`, `PCEncoder`, `SemanticBackbone`, `IlluminationNormStream`, `StructuralSemanticFusion` from the package and branches forward path on `ablation ∈ {full, pc_only, backbone_only, no_fusion}` (matches cell 34 of `PhasePhyto_Colab.ipynb`). (5) `ValDualTransform` with CLAHE + optional HSV leaf-mask (`off`, `hsv`, `hsv_blur`) so each checkpoint's embedded config (v0.1.17+) drives its own val transform via `USE_LEAF_MASK_FROM_CONFIG` (cell 11). (6) `_discover_images` walks any PlantDoc/Cassava root and auto-prefers `test/`/`Test/`/`val/`/`valid/` subdir. `BatchInferenceDataset` handles `UnidentifiedImageError` gracefully with an `is_bad` sentinel flag. `_map_folder_to_source_class` uses exact hit -> `PLANTDOC_TO_PLANTVILLAGE` alias -> `normalize_class_name` fallback. (7) Per-checkpoint inference loop with optional hflip TTA, writes `per_model/<name>_predictions.csv` per model + a wide-format `all_models_predictions.csv` merged on `(image_path, folder)`. (8) `cross_model_agreement.csv` pairwise top-1 agreement matrix, per-class top-1 confidence histograms with 0.5/0.7/0.9 thresholds overlaid, per-image Shannon disagreement entropy across all models as an OOD proxy. (9) `RUN_SUMMARY.md` + `model_meta.json` written at end with per-model mapped accuracy, cross-model agreement pointer, and "Next steps" tying back to `PAPER.md` Limitations (pseudo-label miscalibration, full-vs-backbone_only noise-vs-signal). No training runs triggered. Build script at `/tmp/build_batch_inference_nb.py` writes JSON atomically with AST-validation on every non-shell-magic code cell; notebook verified (`nbformat 4.4`, 28 cells, 0 AST errors). Also updated `notebooks/README.md` Quick-Order step 6 + new "Step 6: Batch inference across all target images" section with fill-in example for `MODEL_CHECKPOINTS`. | `notebooks/PhasePhyto_Batch_Inference.ipynb` (new), `notebooks/README.md`, `CLAUDE.md` |
...
| 0.1.19 | 2026-04-22 | **Cassava Leaf Disease dataset integration (staging only, no training runs).** Motivation: user wants an orthogonal botanical dataset available for future experiments after the PV->PD close-out resumes. Scope constraint: Phase 7.2 close-out experiments remain ON HOLD per 2026-04-22 user decision; this commit adds only the data/loader/notebook plumbing so a future session can train without re-integrating. **Three phases, ≤5 files each, verify between phases.** **Phase 1 (downloader):** Added `download_cassava()`, `_cassava_disease_to_class_name()`, `CASSAVA_LABEL_TO_CLASS` to `scripts/download_data.py` with new `--dataset cassava` CLI choice. Downloads via `kaggle competitions download -c cassava-leaf-disease-classification`, extracts, reads `train.csv` + `label_num_to_disease_map.json` (with fallback to built-in 5-label map), reorganizes Kaggle's flat `train_images/` into `<root>/cassava/Cassava___<Disease>/<image>.jpg` (PlantVillage-compatible ImageFolder). Extended `notebooks/PhasePhyto_Download_Data_To_Drive.ipynb` (10 -> 11 cells): new CONFIG keys `download_cassava` (default False, ~5.5 GB + rules acceptance gate) and `cassava_kaggle_competition`, new `CASSAVA_DIR`, helper + fallback label map, extended Kaggle-creds gate, new Cassava download+reorg cell after PlantDoc, extended tar archive iteration (produces `cassava.tar`), manifest keys (`cassava_dir` / `cassava_num_classes` / `cassava_num_images` / `cassava_classes`), commented `CONFIG['cassava_dir']` path hint. Rules-acceptance URL is printed on 403s. **Phase 2 (loaders):** Added `CassavaDataset(PlantDiseaseDataset)` thin subclass in `phasephyto/data/datasets.py` with `EXPECTED_CLASSES = ("Cassava___Bacterial_Blight", "Cassava___Brown_Streak_Disease", "Cassava___Green_Mottle", "Cassava___Mosaic_Disease", "Cassava___healthy")`. Registered `"cassava": CassavaDataset` in `DATASET_MAP` in both `phasephyto/train.py` and `phasephyto/evaluate.py`. Exported `CassavaDataset` from `phasephyto/data/__init__.py`. Noted Cassava-is-disjoint-from-PV in `phasephyto/data/class_mapping.py` module docstring (no mapping needed). Added `tests/test_cassava_dataset.py` with three tests: ImageFolder load + class count, `DATASET_MAP["cassava"]` dispatch, 5-class sanity. **Phase 3 (training notebook):** Added `CONFIG["dataset"]` selector (default `"plantvillage"`, new `"cassava"`) and `CONFIG["cassava_dir"]` in `notebooks/PhasePhyto_Colab.ipynb` cell 8. Branched tar-hydration in cell 12 (PV+PD vs Cassava) and existence-prints in cell 13. Cell 20 data construction branches: PV keeps 85/15 random_split; Cassava uses stratified-by-class 80/10/10 split (manual per-class shuffle under `random.Random(SEED)` so each class is split proportionally) with the 10% held-out wrapped in `TransformSubset(val_tf)` and assigned to `target_dataset` so downstream eval plumbing runs unchanged. `eval_protocol_label` is set to `"OOD (PlantDoc)"` or `"in-distribution held-out (Cassava)"` and printed in the summary. Cell 21 (PlantDoc class-mapping audit) wrapped in `if CONFIG["dataset"] == "plantvillage":`; cassava branch sets `mapped_target_root = None` and prints a skip message. Cell 22 target-rebuild wrapped in the same guard; cassava branch keeps cell 20's held-out target and creates `target_loader`. Cell 47 eval heading is context-aware ("DOMAIN SHIFT EVALUATION" vs "IN-DISTRIBUTION HELD-OUT EVALUATION") and `phasephyto_domain_shift.json` now includes `dataset` + `eval_protocol` keys. **Critical scope notes:** (a) Cassava numbers report in-distribution test performance, NOT OOD transfer. Explicitly labeled in notebook + JSON. Do not publish as OOD. (b) No training runs executed. Phase 7.2 hold intact. (c) Synthetic mode does not generate Cassava data (PV only). (d) PlantVillage path untouched behaviorally -- `dataset="plantvillage"` reproduces prior runs byte-for-byte in control flow. **How to activate in Colab:** (i) Run `PhasePhyto_Download_Data_To_Drive.ipynb` with `download_cassava=True` to create `cassava.tar` in Drive. (ii) In `PhasePhyto_Colab.ipynb` cell 10 append `CONFIG["dataset"] = "cassava"`. (iii) Run top-to-bottom. Expected: cell 13 prints 5 class dirs with counts; cell 20 prints `Train: ~16800 | Val: ~2100 | Target: ~2100 (in-distribution held-out (Cassava))`; cell 21 prints skip message; cell 47 heading reads "IN-DISTRIBUTION HELD-OUT EVALUATION". **Verify:** ruff + mypy (29 files) + pytest (34 passed, 3 new in `test_cassava_dataset.py`) + notebook AST parse all 63 cells (0 errors) + notebook JSON structure preserved. | `scripts/download_data.py`, `notebooks/PhasePhyto_Download_Data_To_Drive.ipynb`, `phasephyto/data/datasets.py`, `phasephyto/data/__init__.py`, `phasephyto/data/class_mapping.py`, `phasephyto/train.py`, `phasephyto/evaluate.py`, `tests/test_cassava_dataset.py` (new), `notebooks/PhasePhyto_Colab.ipynb` (cells 8, 12, 13, 20, 21, 22, 47), `README.md`, `CLAUDE.md`, `ROADMAP.md`, memory `project_phasephyto.md` |
